...
{
private:
    OnSetParametersCallbackHandle::SharedPtr _parameter_callback_handle;
...
public:
    ...
    {
        this->declare_parameter("_k", 1.0);
        this->declare_parameter("_max_speed", 3.0);

        this->get_parameter("_k", _k);
        this->get_parameter("_max_speed", _max_speed);

        // 改变自身参数的值
        // this->set_parameter(rclcpp::Parameter("_k", 2.0));

        this->_parameter_callback_handle = this->add_on_set_parameters_callback([&](const std::vector<rclcpp::Parameter> & parameters)->rcl_interfaces::msg::SetParametersResult{
            rcl_interfaces::msg::SetParametersResult result;
            result.successful = true;
            for (const auto & parameter : parameters) {
                RCLCPP_INFO(this->get_logger(), "更新参数%s的值为%f",parameter.get_name().c_str(), parameter.as_double());
                if(parameter.get_name() == "_k")
                {
                    _k = parameter.as_double();
                }
                if(parameter.get_name() == "_max_speed")
                {
                    _max_speed = parameter.as_double();
                }
            }
            return result;
        });

        _patrol_service = this->create_service<Patrol>("patrol", [&](const Patrol::Request::SharedPtr request, Patrol::Response::SharedPtr response)->void{
