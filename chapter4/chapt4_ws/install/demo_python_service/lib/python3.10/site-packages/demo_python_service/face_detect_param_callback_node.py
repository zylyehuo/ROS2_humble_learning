...
class FaceDetectNode(Node):
    def __init__(self, node_name):
        ...
        # self._model='hog'
        self.declare_parameter("_number_of_times_to_upsample", 1)
        self.declare_parameter("_model", 'hog')
        self._number_of_times_to_upsample = self.get_parameter("_number_of_times_to_upsample").value
        self._model = self.get_parameter("_model").value
    ...
    def parameters_callback(self, parameters):
        for parameter in parameters:
            ...
            if parameter.name == "_number_of_times_to_upsample":
                self._number_of_times_to_upsample = parameter.value
            if parameter.name == "_model":
                self._model = parameter.value
...
def main():
    rclpy.init()
    node = FaceDetectNode('face_detect_param_callback_node')
    rclpy.spin(node)
    rclpy.shutdown()
