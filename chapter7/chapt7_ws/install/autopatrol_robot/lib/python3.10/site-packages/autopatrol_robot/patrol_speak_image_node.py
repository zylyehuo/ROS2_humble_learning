...
from sensor_msgs.msg import Image  # 图像的消息接口
from cv_bridge import CvBridge  # 图像格式转换
import cv2  # 使用 OpenCV 进行图像的保存
...
class PatrolNode(BasicNavigator):
    def __init__(self, node_name='patrol_node'):
        ...
        # 订阅与保存图像相关定义
        self.declare_parameter('image_save_path', '')  # 图像保存地址，为空：表示保存到当前的相对目录下
        self.image_save_path_ = self.get_parameter('image_save_path').value
        self.cv_bridge_ = CvBridge()
        self.latest_image_ = None  # 用于记录最新的图像
        self.subscription_image_ = self.create_subscription(Image, '/camera_sensor/image_raw', self.image_callback, 1)  # 图像话题的订阅者

    def image_callback(self, msg):
        """
          将最新的图像放到 latest_image_ 中
        """
    ...
    def save_image(self):
        ...
        if self.latest_image_ is not None:
          ...
          cv_image = self.cv_bridge_.imgmsg_to_cv2(self.latest_image_)  # 将 ROS 中图像的消息格式转换为 OpenCV 格式的图像
          cv2.imwrite(f'{self.image_save_path_}image_{pose.translation.x:3.2f}_{pose.translation.y:3.2f}.png', cv_image)  # 保存图像
...
def main():
    ...
    while rclpy.ok():
        ...
        for point in points:
            ...
            patrol.speech_text(text=f'已经到达目标点{x},{y}, 正在准备保存图像')
            patrol.save_image()
            patrol.speech_text(text=f'图像保存完成')
