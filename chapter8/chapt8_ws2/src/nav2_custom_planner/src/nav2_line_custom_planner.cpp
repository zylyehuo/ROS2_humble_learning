...
#include "nav2_custom_planner/nav2_line_custom_planner.hpp"

namespace nav2_line_custom_planner
{

    void LineCustomPlanner::configure(
    ...
    void LineCustomPlanner::cleanup()
    {
        RCLCPP_INFO(node_->get_logger(), "正在清理类型为 LineCustomPlanner 的插件 %s", name_.c_str());
    }

    void LineCustomPlanner::activate()
    {
        RCLCPP_INFO(node_->get_logger(), "正在激活类型为 LineCustomPlanner 的插件 %s", name_.c_str());
    }

    void LineCustomPlanner::deactivate()
    {
        RCLCPP_INFO(node_->get_logger(), "正在停用类型为 LineCustomPlanner 的插件 %s", name_.c_str());
    }

    nav_msgs::msg::Path
    LineCustomPlanner::createPlan(const geometry_msgs::msg::PoseStamped &start, const geometry_msgs::msg::PoseStamped &goal)
    {
        ...
        {
            unsigned int mx, my;
            if (costmap_->worldToMap(pose.pose.position.x, pose.pose.position.y, mx, my))  // 将点的坐标转换为栅格坐标
            {
                unsigned char cost = costmap_->getCost(mx, my); // 获取对应栅格的代价值
                // 如果存在致命障碍物则抛出异常
                if (cost == nav2_costmap_2d::LETHAL_OBSTACLE)
                {
                    RCLCPP_WARN(node_->get_logger(),"在(%f,%f)检测到致命障碍物，规划失败。", pose.pose.position.x, pose.pose.position.y);
...
PLUGINLIB_EXPORT_CLASS(nav2_line_custom_planner::LineCustomPlanner, nav2_core::GlobalPlanner)  // 做导出，给动态链接库做锚点
